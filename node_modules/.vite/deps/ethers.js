import { __export } from "./chunk-RX9MbeNM.js";
...
function _getBytes(value, name, copy$3) {
	if (value instanceof Uint8Array) {
		if (copy$3) return new Uint8Array(value);
		return value;
	}
...
function dataSlice(data, start, end) {
	const bytes$1 = getBytes(data);
	if (end != null && end > bytes$1.length) assert(false, "cannot slice beyond data bounds", "BUFFER_OVERRUN", {
		buffer: bytes$1,
		length: bytes$1.length,
		offset: end
	});
	return hexlify(bytes$1.slice(start == null ? 0 : start, end == null ? bytes$1.length : end));
}
/**
...
function stripZerosLeft(data) {
	let bytes$1 = hexlify(data).substring(2);
	while (bytes$1.startsWith("00")) bytes$1 = bytes$1.substring(2);
	return "0x" + bytes$1;
}
function zeroPad(data, length, left) {
	const bytes$1 = getBytes(data);
	assert(length >= bytes$1.length, "padding exceeds data length", "BUFFER_OVERRUN", {
		buffer: new Uint8Array(bytes$1),
		length,
		offset: length + 1
	});
	const result = new Uint8Array(length);
	result.fill(0);
	if (left) result.set(bytes$1, length - bytes$1.length);
	else result.set(bytes$1, 0);
...
function fromTwos(_value, _width) {
	...
	if (value >> width - BN_1$5) {
		const mask$1 = (BN_1$5 << width) - BN_1$5;
		return -((~value & mask$1) + BN_1$5);
	}
	return value;
}
/**
...
function toBeHex(_value, _width) {
	let result = getUint(_value, "value").toString(16);
	if (_width == null) {
...
function toBeArray(_value) {
	const value = getUint(_value, "value");
	if (value === BN_0$10) return new Uint8Array([]);
...
function encodeBase58(_value) {
	const bytes$1 = getBytes(_value);
	let value = toBigInt(bytes$1);
	let result = "";
	while (value) {
...
function errorFunc(reason, offset, bytes$1, output$1, badCodepoint) {
	assertArgument(false, `invalid codepoint at offset ${offset}; ${reason}`, "bytes", bytes$1);
}
function ignoreFunc(reason, offset, bytes$1, output$1, badCodepoint) {
	if (reason === "BAD_PREFIX" || reason === "UNEXPECTED_CONTINUE") {
		let i = 0;
		for (let o = offset + 1; o < bytes$1.length; o++) {
			if (bytes$1[o] >> 6 !== 2) break;
			i++;
		}
		return i;
	}
	if (reason === "OVERRUN") return bytes$1.length - offset - 1;
	return 0;
}
function replaceFunc(reason, offset, bytes$1, output$1, badCodepoint) {
	if (reason === "OVERLONG") {
		assertArgument(typeof badCodepoint === "number", "invalid bad code point for replacement", "badCodepoint", badCodepoint);
		output$1.push(badCodepoint);
		return 0;
	}
	output$1.push(65533);
	return ignoreFunc(reason, offset, bytes$1, output$1, badCodepoint);
}
/**
...
function getUtf8CodePoints(_bytes, onError) {
	...
	const bytes$1 = getBytes(_bytes, "bytes");
	const result = [];
	let i = 0;
	while (i < bytes$1.length) {
		...
		if ((c & 224) === 192) {
			...
			if ((c & 192) === 128) i += onError("UNEXPECTED_CONTINUE", i - 1, bytes$1, result);
			else i += onError("BAD_PREFIX", i - 1, bytes$1, result);
			continue;
		}
		if (i - 1 + extraLength >= bytes$1.length) {
			i += onError("OVERRUN", i - 1, bytes$1, result);
			continue;
		}
		...
		for (let j = 0; j < extraLength; j++) {
			...
			if ((nextChar & 192) != 128) {
				i += onError("MISSING_CONTINUE", i, bytes$1, result);
				res = null;
				break;
			}
		...
		if (res > 1114111) {
			i += onError("OUT_OF_RANGE", i - 1 - extraLength, bytes$1, result, res);
			continue;
		}
		if (res >= 55296 && res <= 57343) {
			i += onError("UTF16_SURROGATE", i - 1 - extraLength, bytes$1, result, res);
			continue;
		}
		if (res <= overlongMask) {
			i += onError("OVERLONG", i - 1 - extraLength, bytes$1, result, res);
			continue;
		}
		result.push(res);
	}
...
*/
function toUtf8String(bytes$1, onError) {
	return _toUtf8String(getUtf8CodePoints(bytes$1, onError));
}
/**
...
function createGetUrl(options) {
	async function getUrl$1(req, _signal) {
		...
		const init$2 = Object.assign({}, options, {
			method: req.method,
		...
		let resp;
		try {
			resp = await fetch(req.url, init$2);
		} catch (_error) {
			clearTimeout(timer);
...
var FetchRequest = class FetchRequest {
	...
	async #send(attempt, expires, delay, _request, _response) {
		...
		if (scheme in Gateways) {
			...
			if (result instanceof FetchResponse) {
				...
				if (this.processFunc) {
					...
					try {
						response$1 = await this.processFunc(req, response$1);
					} catch (error) {
						if (error.throttle == null || typeof error.stall !== "number") response$1.makeServerError("error in post-processing function", error).assertOk();
					}
				}
				return response$1;
			}
		...
		if (response.statusCode === 301 || response.statusCode === 302) {
			...
			if (this.retryFunc == null || await this.retryFunc(req, response, attempt)) {
				...
				let delay$1 = this.#throttle.slotInterval * Math.trunc(Math.random() * Math.pow(2, attempt));
				if (typeof retryAfter === "string" && retryAfter.match(/^[1-9][0-9]*$/)) delay$1 = parseInt(retryAfter);
				return req.clone().#send(attempt + 1, expires, delay$1, _request, response);
			}
		}
		if (this.processFunc) {
			checkSignal(_request.#signal);
			try {
				...
				let delay$1 = this.#throttle.slotInterval * Math.trunc(Math.random() * Math.pow(2, attempt));
				if (error.stall >= 0) delay$1 = error.stall;
				return req.clone().#send(attempt + 1, expires, delay$1, _request, response);
			}
		}
		return response;
	...
	*/
	redirect(location) {
		...
		assert(this.method === "GET" && (current !== "https" || target !== "http") && location.match(/^https?:/), `unsupported redirect`, "UNSUPPORTED_OPERATION", { operation: `redirect(${this.method} ${JSON.stringify(this.url)} => ${JSON.stringify(location)})` });
	...
	*  throws.
	*/
	static registerGetUrl(getUrl$1) {
		if (locked$5) throw new Error("gateways locked");
		defaultGetUrlFunc = getUrl$1;
	}
	/**
...
var FetchResponse = class FetchResponse {
	...
	*  milliseconds.
	*/
	throwThrottleError(message, stall$4) {
		if (stall$4 == null) stall$4 = -1;
		else assertArgument(Number.isInteger(stall$4) && stall$4 >= 0, "invalid stall timeout", "stall", stall$4);
...
function getFormat(value) {
	...
	let width = 128;
	let decimals = 18;
	if (typeof value === "string") if (value === "fixed") {} else if (value === "ufixed") signed$1 = false;
	else {
		...
		signed$1 = check("signed", "boolean", signed$1);
		width = check("width", "number", width);
		decimals = check("decimals", "number", decimals);
	}
	assertArgument(width % 8 === 0, "invalid FixedNumber width (not byte aligned)", "format.width", width);
	assertArgument(decimals <= 80, "invalid FixedNumber decimals (too large)", "format.decimals", decimals);
	const name = (signed$1 ? "" : "u") + "fixed" + String(width) + "x" + String(decimals);
	return {
		signed: signed$1,
		width,
		decimals,
...
var FixedNumber = class FixedNumber {
	...
	*/
	cmp(other) {
		let a = this.value, b$1 = other.value;
		const delta = this.decimals - other.decimals;
		if (delta > 0) b$1 *= getTens(delta);
...
function _decode(data, offset) {
	...
	const checkOffset = (offset$1) => {
		assert(offset$1 <= data.length, "data short segment too short", "BUFFER_OVERRUN", {
			buffer: data,
			length: data.length,
			offset: offset$1
		});
...
function _encode(object$1) {
	if (Array.isArray(object$1)) {
		let payload = [];
		object$1.forEach(function(child) {
	...
	const data = Array.prototype.slice.call(getBytes(object$1, "object"));
...
*/
function encodeRlp(object$1) {
	let result = "0x";
	for (const v of _encode(object$1)) {
		result += nibbles[v >> 4];
		result += nibbles[v & 15];
	}
	return result;
...
*/
function uuidV4(randomBytes$3) {
	const bytes$1 = getBytes(randomBytes$3, "randomBytes");
	bytes$1[6] = bytes$1[6] & 15 | 64;
	bytes$1[8] = bytes$1[8] & 63 | 128;
	const value = hexlify(bytes$1);
	return [
...
//#endregion
//#region ../node_modules/ethers/lib.esm/abi/coders/abstract-coder.js
/**
* @_ignore:
*/
const WordSize = 32;
...
function setNames(result, names$1) {
	resultNames.set(result, names$1);
}
...
function toObject(names$1, items, deep) {
	if (names$1.indexOf(null) >= 0) return items.map((item, index) => {
		if (item instanceof Result) return toObject(getNames(item), item, deep);
		return item;
	});
	return names$1.reduce((accum, name, index) => {
...
var Result = class Result extends Array {
	...
	*/
	constructor(...args) {
		...
		const nameCounts = names$1.reduce((accum, name) => {
			if (typeof name === "string") accum.set(name, (accum.get(name) || 0) + 1);
			return accum;
		}, /* @__PURE__ */ new Map());
		setNames(this, Object.freeze(items.map((item, index) => {
			const name = names$1[index];
		...
		const proxy = new Proxy(this, { get: (target, prop, receiver) => {
			if (typeof prop === "string") {
				...
				if (value instanceof Function) return function(...args$1) {
					return value.apply(this === receiver ? target : this, args$1);
				};
	...
	*/
	slice(start, end) {
		...
		const result = [], names$1 = [];
		for (let i = start; i < end; i++) {
			result.push(this[i]);
			names$1.push(_names[i]);
		}
		return new Result(_guard$4, result, names$1);
	}
	/**
	*  @_ignore
	*/
...
function checkResultErrors(result) {
	const errors = [];
	const checkErrors = function(path, object$1) {
		if (!Array.isArray(object$1)) return;
		for (let key in object$1) {
			const childPath = path.slice();
			childPath.push(key);
			try {
				checkErrors(childPath, object$1[key]);
			} catch (error) {
				errors.push({
					path: childPath,
					error
...
function getValue$1(value) {
	let bytes$1 = toBeArray(value);
	assert(bytes$1.length <= WordSize, "value out-of-bounds", "BUFFER_OVERRUN", {
		buffer: bytes$1,
		length: WordSize,
		offset: bytes$1.length
	});
	if (bytes$1.length !== WordSize) bytes$1 = getBytesCopy(concat([Padding.slice(bytes$1.length % WordSize), bytes$1]));
	return bytes$1;
}
/**
*  @_ignore
...
var Writer = class {
	...
	writeBytes(value) {
		let bytes$1 = getBytesCopy(value);
		const paddingOffset = bytes$1.length % WordSize;
		if (paddingOffset) bytes$1 = getBytesCopy(concat([bytes$1, Padding.slice(paddingOffset)]));
		return this.#writeData(bytes$1);
	}
	writeValue(value) {
...
var Reader = class Reader {
	...
	readBytes(length, loose) {
		let bytes$1 = this.#peekBytes(0, length, !!loose);
		this.#incrementBytesRead(length);
		this.#offset += bytes$1.length;
		return bytes$1.slice(0, length);
	}
	readValue() {
...
function number(n$1) {
	if (!Number.isSafeInteger(n$1) || n$1 < 0) throw new Error(`Wrong positive integer: ${n$1}`);
}
function bytes(b$1, ...lengths) {
	if (!(b$1 instanceof Uint8Array)) throw new Error("Expected Uint8Array");
	if (lengths.length > 0 && !lengths.includes(b$1.length)) throw new Error(`Expected Uint8Array of length ${lengths}, not of length=${b$1.length}`);
}
function hash(hash$1) {
	if (typeof hash$1 !== "function" || typeof hash$1.create !== "function") throw new Error("Hash should be wrapped by utils.wrapConstructor");
	number(hash$1.outputLen);
	number(hash$1.blockLen);
}
...
*/
function pbkdf2$1(hash$1, password, salt, opts) {
...
var Chi = (a, b$1, c) => a & b$1 ^ ~a & c;
var Maj = (a, b$1, c) => a & b$1 ^ a & c ^ b$1 & c;
...
var SHA256 = class extends SHA2 {
	...
	process(view, offset) {
		...
		for (let i = 0; i < 64; i++) {
			...
			const T1$2 = H + sigma1 + Chi(E, F, G) + SHA256_K[i] + SHA256_W[i] | 0;
			const T2$1 = (rotr(A, 2) ^ rotr(A, 13) ^ rotr(A, 22)) + Maj(A, B, C) | 0;
			H = G;
			G = F;
			F = E;
			E = D + T1$2 | 0;
...
var SHA512 = class extends SHA2 {
	...
	process(view, offset) {
		...
		for (let i = 16; i < 80; i++) {
			...
			SHA512_W_H[i] = _u64_default.add4H(SUMl, s0h, s1h, SHA512_W_H[i - 7], SHA512_W_H[i - 16]) | 0;
			SHA512_W_L[i] = SUMl | 0;
		}
...
function randomBytes$2(length) {
	assert(crypto != null, "platform does not support secure random numbers", "UNSUPPORTED_OPERATION", { operation: "randomBytes" });
...
function scryptInit(password, salt, _opts) {
	const { N: N$2, r, p, dkLen, asyncTick, maxmem, onProgress } = checkOpts({
		dkLen: 32,
		asyncTick: 10,
		maxmem: 1024 ** 3 + 1024
	}, _opts);
	...
	if (N$2 <= 1 || (N$2 & N$2 - 1) !== 0 || N$2 >= 2 ** (blockSize / 8) || N$2 > 2 ** 32) throw new Error("Scrypt: N must be larger than 1, a power of 2, less than 2^(128 * r / 8) and less than 2^32");
	...
	const B32 = u32(B);
	const V = u32(new Uint8Array(blockSize * N$2));
	const tmp = u32(new Uint8Array(blockSize));
	let blockMixCb = () => {};
	if (onProgress) {
		const totalBlockMix = 2 * N$2 * p;
...
function scrypt$1(password, salt, opts) {
	const { N: N$2, r, p, dkLen, blockSize32, V, B32, B, tmp, blockMixCb } = scryptInit(password, salt, opts);
	for (let pi = 0; pi < p; pi++) {
		const Pi$1 = blockSize32 * pi;
		for (let i = 0; i < blockSize32; i++) V[i] = B32[Pi$1 + i];
		for (let i = 0, pos = 0; i < N$2 - 1; i++) {
			BlockMix(V, pos, V, pos += blockSize32, r);
			blockMixCb();
		}
		BlockMix(V, (N$2 - 1) * blockSize32, B32, Pi$1, r);
		blockMixCb();
		for (let i = 0; i < N$2; i++) {
			const j = B32[Pi$1 + blockSize32 - 16] % N$2;
			for (let k = 0; k < blockSize32; k++) tmp[k] = B32[Pi$1 + k] ^ V[j * blockSize32 + k];
			BlockMix(tmp, 0, B32, Pi$1, r);
			blockMixCb();
		}
	}
...
async function scryptAsync(password, salt, opts) {
	const { N: N$2, r, p, dkLen, blockSize32, V, B32, B, tmp, blockMixCb, asyncTick } = scryptInit(password, salt, opts);
	for (let pi = 0; pi < p; pi++) {
		const Pi$1 = blockSize32 * pi;
		for (let i = 0; i < blockSize32; i++) V[i] = B32[Pi$1 + i];
		let pos = 0;
		await asyncLoop(N$2 - 1, asyncTick, () => {
			BlockMix(V, pos, V, pos += blockSize32, r);
			blockMixCb();
		});
		BlockMix(V, (N$2 - 1) * blockSize32, B32, Pi$1, r);
		blockMixCb();
		await asyncLoop(N$2, asyncTick, () => {
			const j = B32[Pi$1 + blockSize32 - 16] % N$2;
			for (let k = 0; k < blockSize32; k++) tmp[k] = B32[Pi$1 + k] ^ V[j * blockSize32 + k];
			BlockMix(tmp, 0, B32, Pi$1, r);
			blockMixCb();
...
var _scryptAsync = async function(passwd, salt, N$2, r, p, dkLen, onProgress) {
...
var _scryptSync = function(passwd, salt, N$2, r, p, dkLen) {
...
*    //_result:
*/
async function scrypt(_passwd, _salt, N$2, r, p, dkLen, progress) {
	const passwd = getBytes(_passwd, "passwd");
	const salt = getBytes(_salt, "salt");
	return hexlify(await __scryptAsync(passwd, salt, N$2, r, p, dkLen, progress));
}
...
*    //_result:
*/
function scryptSync(_passwd, _salt, N$2, r, p, dkLen) {
	const passwd = getBytes(_passwd, "passwd");
	const salt = getBytes(_salt, "salt");
	return hexlify(__scryptSync(passwd, salt, N$2, r, p, dkLen));
}
...
function bytesToHex(bytes$1) {
	if (!u8a(bytes$1)) throw new Error("Uint8Array expected");
	let hex = "";
...
function bytesToNumberBE(bytes$1) {
	return hexToNumber(bytesToHex(bytes$1));
}
function bytesToNumberLE(bytes$1) {
	if (!u8a(bytes$1)) throw new Error("Uint8Array expected");
	return hexToNumber(bytesToHex(Uint8Array.from(bytes$1).reverse()));
}
function numberToBytesBE(n$1, len) {
	return hexToBytes(n$1.toString(16).padStart(len * 2, "0"));
}
function numberToBytesLE(n$1, len) {
	return numberToBytesBE(n$1, len).reverse();
}
function numberToVarBytesBE(n$1) {
	return hexToBytes(numberToHexUnpadded(n$1));
}
/**
...
function bitLen(n$1) {
	let len;
	for (len = 0; n$1 > _0n$4; n$1 >>= _1n$4, len += 1);
	return len;
}
/**
...
*/
function bitGet(n$1, pos) {
	return n$1 >> BigInt(pos) & _1n$4;
}
/**
* Sets single bit at position.
*/
const bitSet = (n$1, pos, value) => {
	return n$1 | (value ? _1n$4 : _0n$4) << BigInt(pos);
};
/**
* Calculate mask for N bits. Not using ** operator with bigints because of old engines.
* Same as BigInt(`0b${Array(i).fill('1').join('')}`)
*/
const bitMask = (n$1) => (_2n$3 << BigInt(n$1 - 1)) - _1n$4;
...
function validateObject(object$1, validators, optValidators = {}) {
	const checkField = (fieldName, type, isOptional) => {
		...
		if (!checkVal(val, object$1)) throw new Error(`Invalid param ${String(fieldName)}=${val} (${typeof val}), expected ${type}`);
	};
...
var _0n$3 = BigInt(0), _1n$3 = BigInt(1), _2n$2 = BigInt(2), _3n$1 = BigInt(3);
var _4n$1 = BigInt(4), _5n = BigInt(5), _8n = BigInt(8);
var _9n = BigInt(9), _16n = BigInt(16);
function mod(a, b$1) {
	const result = a % b$1;
	return result >= _0n$3 ? result : b$1 + result;
}
/**
...
function invert(number$1, modulo) {
	...
	while (a !== _0n$3) {
		...
		const m = x - u * q;
		const n$1 = y - v * q;
		b$1 = a, a = r, x = u, y = v, u = m, v = n$1;
	}
	if (b$1 !== _1n$3) throw new Error("invert: does not exist");
	return mod(x, modulo);
}
/**
...
function tonelliShanks(P) {
	...
	let Q, S$1, Z;
	for (Q = P - _1n$3, S$1 = 0; Q % _2n$2 === _0n$3; Q /= _2n$2, S$1++);
	for (Z = _2n$2; Z < P && pow(Z, legendreC, P) !== P - _1n$3; Z++);
	if (S$1 === 1) {
		const p1div4 = (P + _1n$3) / _4n$1;
		return function tonelliFast(Fp$1, n$1) {
			const root = Fp$1.pow(n$1, p1div4);
			if (!Fp$1.eql(Fp$1.sqr(root), n$1)) throw new Error("Cannot find square root");
			return root;
		};
	}
	const Q1div2 = (Q + _1n$3) / _2n$2;
	return function tonelliSlow(Fp$1, n$1) {
		if (Fp$1.pow(n$1, legendreC) === Fp$1.neg(Fp$1.ONE)) throw new Error("Cannot find square root");
		let r = S$1;
		let g = Fp$1.pow(Fp$1.mul(Fp$1.ONE, Z), Q);
		let x = Fp$1.pow(n$1, Q1div2);
		let b$1 = Fp$1.pow(n$1, Q);
		while (!Fp$1.eql(b$1, Fp$1.ONE)) {
			if (Fp$1.eql(b$1, Fp$1.ZERO)) return Fp$1.ZERO;
			let m = 1;
			for (let t2 = Fp$1.sqr(b$1); m < r; m++) {
...
function FpSqrt(P) {
	if (P % _4n$1 === _3n$1) {
		const p1div4 = (P + _1n$3) / _4n$1;
		return function sqrt3mod4(Fp$1, n$1) {
			const root = Fp$1.pow(n$1, p1div4);
			if (!Fp$1.eql(Fp$1.sqr(root), n$1)) throw new Error("Cannot find square root");
			return root;
		};
	...
	if (P % _8n === _5n) {
		const c1 = (P - _5n) / _8n;
		return function sqrt5mod8(Fp$1, n$1) {
			const n2 = Fp$1.mul(n$1, _2n$2);
			const v = Fp$1.pow(n2, c1);
			const nv = Fp$1.mul(n$1, v);
...
* Unsafe in some contexts: uses ladder, so can expose bigint bits.
*/
function FpPow(f$1, num, power) {
	if (power < _0n$3) throw new Error("Expected power > 0");
	if (power === _0n$3) return f$1.ONE;
	if (power === _1n$3) return num;
	let p = f$1.ONE;
	let d = num;
	while (power > _0n$3) {
		if (power & _1n$3) p = f$1.mul(p, d);
		d = f$1.sqr(d);
		power >>= _1n$3;
	}
	return p;
...
*/
function FpInvertBatch(f$1, nums) {
	const tmp = new Array(nums.length);
	const lastMultiplied = nums.reduce((acc, num, i) => {
		if (f$1.is0(num)) return acc;
		tmp[i] = acc;
		return f$1.mul(acc, num);
	}, f$1.ONE);
	const inverted = f$1.inv(lastMultiplied);
	nums.reduceRight((acc, num, i) => {
		if (f$1.is0(num)) return acc;
		tmp[i] = f$1.mul(acc, tmp[i]);
		return f$1.mul(acc, num);
	}, inverted);
	return tmp;
}
function nLength(n$1, nBitLength) {
	const _nBitLength = nBitLength !== void 0 ? nBitLength : n$1.toString(2).length;
...
function Field(ORDER, bitLen$1, isLE$1 = false, redef = {}) {
	...
	const f$1 = Object.freeze({
		...
		sqrt: redef.sqrt || ((n$1) => sqrtP(f$1, n$1)),
...
function wNAF(c, bits) {
	...
	return {
		constTimeNegate,
		unsafeLadder(elm, n$1) {
			let p = c.ZERO;
			let d = elm;
			while (n$1 > _0n$2) {
				if (n$1 & _1n$2) p = p.add(d);
				d = d.double();
				n$1 >>= _1n$2;
			}
			return p;
		...
		wNAF(W, precomputes, n$1) {
			...
			for (let window$1 = 0; window$1 < windows; window$1++) {
				...
				const cond2 = wbits < 0;
				if (wbits === 0) f$1 = f$1.add(constTimeNegate(cond1, precomputes[offset1]));
		...
		wNAFCached(P, precomputesMap, n$1, transform) {
...
function weierstrassPoints(opts) {
	...
	const toBytes$1 = CURVE.toBytes || ((_c, point, _isCompressed) => {
		...
		return concatBytes$1(Uint8Array.from([4]), Fp$1.toBytes(a.x), Fp$1.toBytes(a.y));
	});
	...
	*/
	function weierstrassEquation(x) {
		...
		return Fp$1.add(Fp$1.add(x3, Fp$1.mul(x, a)), b$1);
	}
	...
	function normPrivateKeyToScalar(key) {
		const { allowedPrivateKeyLengths: lengths, nByteLength, wrapPrivateKey, n: n$1 } = CURVE;
		...
		if (wrapPrivateKey) num = mod(num, n$1);
		assertGE(num);
		return num;
	}
	...
	class Point$1 {
		...
		*/
		equals(other) {
			...
			const U1$1 = Fp$1.eql(Fp$1.mul(X1, Z2), Fp$1.mul(X2, Z1));
			const U2$1 = Fp$1.eql(Fp$1.mul(Y1, Z2), Fp$1.mul(Y2, Z1));
			return U1$1 && U2$1;
		}
		/**
		...
		*/
		...
		wNAF(n$1) {
			return wnaf.wNAFCached(this, pointPrecomputes, n$1, (comp) => {
		...
		*/
		multiplyUnsafe(n$1) {
			...
			if (n$1 === _0n$1) return I;
			assertGE(n$1);
			if (n$1 === _1n$1) return this;
			const { endo } = CURVE;
			if (!endo) return wnaf.unsafeLadder(this, n$1);
			let { k1neg, k1, k2neg, k2 } = endo.splitScalar(n$1);
			let k1p = I;
			let k2p = I;
			let d = this;
		...
		*/
		multiply(scalar) {
			...
			const { endo } = CURVE;
			if (endo) {
				const { k1neg, k1, k2neg, k2 } = endo.splitScalar(n$1);
		...
		*/
		multiplyAndAddUnsafe(Q, a, b$1) {
			...
			const sum = mul(this, a).add(mul(Q, b$1));
...
function weierstrass(curveDef) {
	...
	const slcNum = (b$1, from, to) => bytesToNumberBE(b$1.slice(from, to));
	/**
	...
	*/
	class Signature$1 {
		...
		static fromCompact(hex) {
			...
			return new Signature$1(slcNum(hex, 0, l), slcNum(hex, l, 2 * l));
		}
		static fromDER(hex) {
		...
		addRecoveryBit(recovery) {
			return new Signature$1(this.r, this.s, recovery);
		}
		...
		normalizeS() {
			return this.hasHighS() ? new Signature$1(this.r, modN(-this.s), this.recovery) : this;
		}
		toDERRawBytes() {
	...
	*/
	...
	function prepSig(msgHash, privateKey, opts = defaultSigOpts) {
		...
		const { hash: hash$1, randomBytes: randomBytes$3 } = CURVE;
		...
		if (ent != null) {
			const e = ent === true ? randomBytes$3(Fp$1.BYTES) : ent;
	...
	function verify(signature, msgHash, publicKey, opts = defaultVerOpts) {
		...
		try {
			if (typeof sg === "string" || sg instanceof Uint8Array) try {
				_sig = Signature$1.fromDER(sg);
			} catch (derError) {
				if (!(derError instanceof DER.Err)) throw derError;
				_sig = Signature$1.fromCompact(sg);
			}
			else if (typeof sg === "object" && typeof sg.r === "bigint" && typeof sg.s === "bigint") {
				const { r: r$1, s: s$1 } = sg;
				_sig = new Signature$1(r$1, s$1);
...
function getHash(hash$1) {
	return {
		hash: hash$1,
		hmac: (key, ...msgs) => hmac(hash$1, key, concatBytes(...msgs)),
		randomBytes: randomBytes$1
	};
}
function createCurve(curveDef, defHash) {
	const create = (hash$1) => weierstrass({
		...curveDef,
		...getHash(hash$1)
	});
	return Object.freeze({
...
var _1n = BigInt(1);
var _2n = BigInt(2);
var divNearest = (a, b$1) => (a + b$1 / _2n) / b$1;
/**
...
const secp256k1 = createCurve({
	...
	endo: {
		...
		splitScalar: (k) => {
			const n$1 = secp256k1N;
			const a1 = BigInt("0x3086d221a7d46bcde86c90e49284eb15");
			const b1 = -_1n * BigInt("0xe4437ed6010e88286f547fa90abfe4c3");
			const a2 = BigInt("0x114ca50f7a8e2f3f657c1108d9d44cfd8");
			const b2 = a1;
			const POW_2_128 = BigInt("0x100000000000000000000000000000000");
			const c1 = divNearest(b2 * k, n$1);
			const c2 = divNearest(-b1 * k, n$1);
			let k1 = mod(k - c1 * a1 - c2 * a2, n$1);
			let k2 = mod(-c1 * b1 - c2 * b2, n$1);
	...
	*/
	static from(sig) {
		...
		if (typeof sig === "string") {
			const bytes$1 = getBytes(sig, "signature");
			if (bytes$1.length === 64) {
				const r$1 = hexlify(bytes$1.slice(0, 32));
				const s$1 = bytes$1.slice(32, 64);
				const v$1 = s$1[0] & 128 ? 28 : 27;
				s$1[0] &= 127;
				return new Signature(_guard$3, r$1, hexlify(s$1), v$1);
			}
			if (bytes$1.length === 65) {
				const r$1 = hexlify(bytes$1.slice(0, 32));
				const s$1 = hexlify(bytes$1.slice(32, 64));
				const v$1 = Signature.getNormalizedV(bytes$1[64]);
				return new Signature(_guard$3, r$1, s$1, v$1);
			}
		...
		const s = (function(s$1, yParityAndS) {
			if (s$1 != null) return toUint256(s$1);
			if (yParityAndS != null) {
				assertError(isHexString(yParityAndS, 32), "invalid yParityAndS");
				const bytes$1 = getBytes(yParityAndS);
				bytes$1[0] &= 127;
				return hexlify(bytes$1);
			}
			assertError(false, "missing s");
		})(sig.s, sig.yParityAndS);
		const { networkV, v } = (function(_v, yParityAndS, yParity) {
			if (_v != null) {
				const v$1 = getBigInt(_v);
				return {
					networkV: v$1 >= BN_35$1 ? v$1 : void 0,
					v: Signature.getNormalizedV(v$1)
				};
			}
			if (yParityAndS != null) {
				assertError(isHexString(yParityAndS, 32), "invalid yParityAndS");
...
var SigningKey = class SigningKey {
	...
	*/
	static computePublicKey(key, compressed) {
		let bytes$1 = getBytes(key, "key");
		if (bytes$1.length === 32) {
			const pubKey = secp256k1.getPublicKey(bytes$1, !!compressed);
		...
		const point = secp256k1.ProjectivePoint.fromHex(bytes$1);
...
function ibanChecksum(address) {
	...
	let checksum$1 = String(98 - parseInt(expanded, 10) % 97);
	while (checksum$1.length < 2) checksum$1 = "0" + checksum$1;
	return checksum$1;
}
var Base36 = (function() {
	const result = {};
...
function n(value, width) {
	...
	return new Typed(_gaurd, `${signed$1 ? "" : "u"}int${width}`, value, {
		signed: signed$1,
		width
	});
}
...
var NumberCoder = class extends Coder {
	size;
	signed;
	constructor(size, signed$1, localName) {
		const name = (signed$1 ? "int" : "uint") + size * 8;
		super(name, name, localName, false);
		defineProperties(this, {
			size,
			signed: signed$1
...
function accessListify(value) {
	...
	result.sort((a, b$1) => a.address.localeCompare(b$1.address));
	return result;
}
...
function getVersionedHash(version$1, hash$1) {
	let versioned = version$1.toString(16);
	while (versioned.length < 2) versioned = "0" + versioned;
	versioned += sha256(hash$1).substring(4);
	return "0x" + versioned;
}
...
function _serializeEip4844(tx, sig, blobs) {
	...
	if (sig) {
		...
		fields.push(toBeArray(sig.r));
		fields.push(toBeArray(sig.s));
		if (blobs) return concat(["0x03", encodeRlp([
			fields,
			blobs.map((b$1) => b$1.data),
			blobs.map((b$1) => b$1.commitment),
			blobs.map((b$1) => b$1.proof)
		])]);
	}
...
var Transaction = class Transaction {
	...
	*/
	get blobs() {
		...
		return this.#blobs.map((b$1) => Object.assign({}, b$1));
	}
	set blobs(_blobs) {
		if (_blobs == null) {
			this.#blobs = null;
	...
	*/
	...
	#getSerialized(signed$1, sidecar) {
		assert(!signed$1 || this.signature != null, "cannot serialize unsigned transaction; maybe you meant .unsignedSerialized", "UNSUPPORTED_OPERATION", { operation: ".serialized" });
		const sig = signed$1 ? this.signature : null;
...
function decode_arithmetic(bytes$1) {
	...
	const MASK = FULL - 1;
	let register = 0;
	for (let i = 0; i < N$2; i++) register = register << 1 | read_bit();
	let symbols = [];
	let low = 0;
	let range = FULL;
	while (true) {
		...
		let b$1 = low + Math.floor(range * acc[start + 1] / total) - 1;
		while (((a ^ b$1) & HALF) == 0) {
...
function unsafe_atob(s) {
	...
	let n$1 = s.length;
	let ret = new Uint8Array(6 * n$1 >> 3);
	for (let i = 0, pos = 0, width = 0, carry = 0; i < n$1; i++) {
...
function read_deltas(n$1, next) {
	let v = Array(n$1);
	for (let i = 0, x = 0; i < n$1; i++) v[i] = x += signed(next());
	return v;
}
function read_sorted(next, prev = 0) {
	...
	while (true) {
		...
		if (!n$1) break;
		prev += x;
		for (let i = 0; i < n$1; i++) ret.push(prev + i);
		prev += n$1 + 1;
	}
	return ret;
}
...
function read_transposed(n$1, w, next) {
	let m = Array(n$1).fill().map(() => []);
	for (let i = 0; i < w; i++) read_deltas(n$1, next).forEach((x, j) => m[j].push(x));
	return m;
}
...
function read_replacement_table(w, next) {
	let n$1 = 1 + next();
	return read_transposed(n$1, 1 + w, next).map((v) => [v[0], v.slice(1)]);
}
function read_trie(next) {
	...
	function expand({ S: S$1, B }, cps, saved) {
		if (S$1 & 4 && saved === cps[cps.length - 1]) return;
		if (S$1 & 2) saved = cps[cps.length - 1];
		if (S$1 & 1) ret.push(cps);
...
function compare_arrays(a, b$1) {
	let n$1 = a.length;
	let c = n$1 - b$1.length;
	for (let i = 0; c == 0 && i < n$1; i++) c = a[i] - b$1[i];
	return c;
}
...
function compose_pair(a, b$1) {
	if (a >= L0 && a < L1 && b$1 >= V0 && b$1 < V1) return S0 + (a - L0) * N_COUNT + (b$1 - V0) * T_COUNT;
	else if (is_hangul(a) && b$1 > T0 && b$1 < T1$1 && (a - S0) % T_COUNT == 0) return a + (b$1 - T0);
	else {
		let recomp = RECOMP.get(a);
		if (recomp) {
			recomp = recomp.get(b$1);
...
function init() {
	...
	GROUPS = read_array_while((i) => {
		let N$2 = read_array_while(r).map((x) => x + 96);
		if (N$2.length) {
			let R = i >= unrestricted;
			N$2[0] -= 32;
			N$2 = str_from_cps(N$2);
			if (R) N$2 = `Restricted[${N$2}]`;
			let P = read_chunked();
			let Q = read_chunked();
			let M = !r();
			return {
	...
	let wholes = read_sorted_array().concat(Array_from(WHOLE_VALID)).sort((a, b$1) => a - b$1);
	wholes.forEach((cp, i) => {
		let d = r();
...
function check_fenced(cps) {
	...
	if (last == n$1) throw error_placement(`trailing ${prev}`);
}
function safe_str_from_cps(cps, max = Infinity, quoter = quote_cp) {
	...
	buf.push(str_from_cps(cps.slice(prev, n$1)));
	return buf.join("");
}
...
function ensNameSplit(name) {
	const bytes$1 = toUtf8Bytes(ensNormalize(name));
	const comps = [];
	if (name.length === 0) return comps;
	let last = 0;
	for (let i = 0; i < bytes$1.length; i++) if (bytes$1[i] === 46) {
		comps.push(checkComponent(bytes$1.slice(last, i)));
		last = i + 1;
	}
	assertArgument(last < bytes$1.length, "invalid ENS name; empty component", "name", name);
	comps.push(checkComponent(bytes$1.slice(last)));
	return comps;
}
/**
...
function dnsEncode(name, _maxLength) {
	...
	return hexlify(concat(ensNameSplit(name).map((comp) => {
		...
		const bytes$1 = new Uint8Array(comp.length + 1);
		bytes$1.set(comp, 1);
		bytes$1[0] = bytes$1.length - 1;
		return bytes$1;
	}))) + "00";
}
...
function _pack(type, value, isArray) {
	...
	if (match) {
		let signed$1 = match[1] === "int";
		let size = parseInt(match[2] || "256");
		assertArgument((!match[2] || match[2] === String(size)) && size % 8 === 0 && size !== 0 && size <= 256, "invalid number type", "type", type);
		if (isArray) size = 256;
		if (signed$1) value = toTwos(value, size);
	...
	if (match && Array.isArray(value)) {
		...
		const result = [];
		value.forEach(function(value$1) {
			result.push(_pack(baseType, value$1, true));
		});
...
function hexPadRight(value) {
	const bytes$1 = getBytes(value);
	const padOffset = bytes$1.length % 32;
	if (padOffset) return concat([bytes$1, padding.slice(padOffset)]);
	return hexlify(bytes$1);
}
...
var domainChecks = {
	...
	salt: function(value) {
		const bytes$1 = getBytes(value, "domain.salt");
		assertArgument(bytes$1.length === 32, `invalid domain value "salt"`, "domain.salt", value);
		return hexlify(bytes$1);
	}
};
function getBaseEncoder(type) {
	{
		...
		if (match) {
			...
			const boundsUpper = mask(BN_MAX_UINT256, signed$1 ? width - 1 : width);
			const boundsLower = signed$1 ? (boundsUpper + BN_1$1) * BN__1 : BN_0$3;
			return function(_value) {
				const value = getBigInt(_value, "value");
				assertArgument(value >= boundsLower && value <= boundsUpper, `value out-of-bounds for ${type}`, "value", value);
				return toBeHex(signed$1 ? toTwos(value, 256) : value, 32);
			};
		}
	}
	{
		...
		if (match) {
			...
			return function(value) {
				const bytes$1 = getBytes(value);
				assertArgument(bytes$1.length === width, `invalid length for ${type}`, "value", value);
...
var TypedDataEncoder = class TypedDataEncoder {
	...
	*/
	constructor(_types) {
		...
		Object.keys(_types).forEach((type) => {
			types[type] = _types[type].map(({ name, type: type$1 }) => {
				let { base, index } = splitArray(type$1);
		...
		const primaryTypes = Array.from(parents.keys()).filter((n$1) => parents.get(n$1).length === 0);
		...
		if (fields) {
			...
			return (value) => {
				const values = fields.map(({ name, type: type$1 }) => {
					...
					if (this.#fullTypes.has(type$1)) return keccak256(result);
					return result;
				});
	...
	*/
	static hashDomain(domain) {
		...
		domainFields.sort((a, b$1) => {
			return domainFieldNames.indexOf(a.name) - domainFieldNames.indexOf(b$1.name);
		});
	...
	static getPayload(domain, types, value) {
		...
		return {
			...
			message: encoder.visit(value, (type, value$1) => {
				if (type.match(/^bytes(\d*)/)) return hexlify(getBytes(value$1));
				if (type.match(/^u?int/)) return getBigInt(value$1).toString();
				switch (type) {
					case "address": return value$1.toLowerCase();
					case "bool": return !!value$1;
					case "string":
						assertArgument(typeof value$1 === "string", "invalid string", "value", value$1);
						return value$1;
				}
	...
	while (offset < text.length) {
		...
		if (type) {
			...
			if (type === "OPEN_PAREN") {
				...
				if (brackets.length === 0) throwError$1("no matching open bracket");
...
var ParamType = class ParamType {
	...
	format(format) {
		...
		if (format === "json") {
			...
			if (this.isArray()) {
				...
				result$2.name = name;
				result$2.type += `[${this.arrayLength < 0 ? "" : String(this.arrayLength)}]`;
			...
			if (this.isTuple()) result$1.components = this.components.map((c) => JSON.parse(c.format(format)));
	...
	*/
	...
	#walkAsync(promises, value, process, setValue) {
		if (this.isArray()) {
			...
			const result$1 = value.slice();
			result$1.forEach((value$1, index) => {
				childType.#walkAsync(promises, value$1, process, (value$2) => {
					result$1[index] = value$2;
				});
			});
			setValue(result$1);
			return;
		...
		if (this.isTuple()) {
			...
			let result$1;
			if (Array.isArray(value)) result$1 = value.slice();
			else {
				if (value == null || typeof value !== "object") throw new Error("invalid tuple value");
				result$1 = components.map((param) => {
			...
			if (result$1.length !== this.components.length) throw new Error("array is wrong length");
			result$1.forEach((value$1, index) => {
				components[index].#walkAsync(promises, value$1, process, (value$2) => {
					result$1[index] = value$2;
				});
			});
			setValue(result$1);
	...
	*/
	async walkAsync(value, process) {
		const promises = [];
		const result = [value];
		this.#walkAsync(promises, value, process, (value$1) => {
			result[0] = value$1;
		});
	...
	static from(obj, allowIndexed) {
		...
		if (typeof obj === "string") try {
			...
			if (consumeKeywords(obj, setify(["tuple"])).has("tuple") || obj.peekType("OPEN_PAREN")) {
				...
				type$1 = `tuple(${comps.map((c) => c.format()).join(",")})`;
			} else {
				type$1 = verifyBasicType(obj.popType("TYPE"));
				baseType = type$1;
			}
			...
			while (obj.length && obj.peekType("BRACKET")) {
				...
				arrayChildren = new ParamType(_guard$2, "", type$1, baseType, null, comps, arrayLength, arrayChildren);
			...
			const name$1 = obj.peekType("ID") ? obj.pop().text : "";
			if (obj.length) throw new Error("leftover tokens");
			return new ParamType(_guard$2, name$1, type$1, baseType, indexed$1, comps, arrayLength, arrayChildren);
		}
		const name = obj.name;
	...
	static from(obj) {
		...
		if (typeof obj === "string") try {
			...
			if (obj.popKeyword(setify(["fallback", "receive"])) === "receive") {
				const inputs$1 = consumeParams(obj);
				assertArgument(inputs$1.length === 0, `receive cannot have arguments`, "obj.inputs", inputs$1);
...
function getBuiltinCallException(action, tx, data, abiCoder) {
	...
	if (data) {
		...
		if (bytes$1.length === 0) {
			...
			const code = Number(abiCoder.decode(["uint256"], bytes$1.slice(4))[0]);
			revert = {
...
var AbiCoder = class AbiCoder {
	...
	*/
	decode(types, data, loose) {
		const coders = types.map((type) => this.#getCoder(ParamType.from(type)));
		return new TupleCoder(coders, "_").decode(new Reader(data, loose, defaultMaxInflation));
	}
...
function encodeBytes32String(text) {
	...
	if (bytes$1.length > 31) throw new Error("bytes32 string must be less than 32 bytes");
	return zeroPadBytes(bytes$1, 32);
}
/**
...
var Interface = class Interface {
	...
	*/
	formatJson() {
		const abi = this.fragments.map((f$1) => f$1.format("json"));
	...
	*/
	forEachFunction(callback) {
		const names$1 = Array.from(this.#functions.keys());
		names$1.sort((a, b$1) => a.localeCompare(b$1));
		for (let i = 0; i < names$1.length; i++) {
			const name = names$1[i];
	...
	*/
	forEachEvent(callback) {
		const names$1 = Array.from(this.#events.keys());
		names$1.sort((a, b$1) => a.localeCompare(b$1));
		for (let i = 0; i < names$1.length; i++) {
			const name = names$1[i];
	...
	*/
	forEachError(callback) {
		const names$1 = Array.from(this.#errors.keys());
		names$1.sort((a, b$1) => a.localeCompare(b$1));
		for (let i = 0; i < names$1.length; i++) {
			const name = names$1[i];
	...
	*/
	decodeErrorResult(fragment, data) {
		if (typeof fragment === "string") {
			const f$1 = this.getError(fragment);
			assertArgument(f$1, "unknown error", "fragment", fragment);
			fragment = f$1;
		}
	...
	*/
	decodeFunctionData(fragment, data) {
		if (typeof fragment === "string") {
			const f$1 = this.getFunction(fragment);
	...
	*/
	...
	encodeFilterTopics(fragment, values) {
		if (typeof fragment === "string") {
			const f$1 = this.getEvent(fragment);
			assertArgument(f$1, "unknown event", "eventFragment", fragment);
			fragment = f$1;
		}
		...
		values.forEach((value, index) => {
			...
			else if (Array.isArray(value)) topics.push(value.map((value$1) => encodeTopic(param, value$1)));
...
function copyRequest(req) {
	...
	if ("blobs" in req && req.blobs) result.blobs = req.blobs.map((b$1) => {
		if (isBytesLike(b$1)) return hexlify(b$1);
		return Object.assign({}, b$1);
	});
	return result;
}
/**
...
var Block = class {
	...
	*/
	toJSON() {
		const { baseFeePerGas, difficulty, extraData, gasLimit, gasUsed, hash: hash$1, miner, prevRandao, nonce, number: number$1, parentHash, parentBeaconBlockRoot, stateRoot, receiptsRoot, timestamp, transactions } = this;
		return {
			_type: "Block",
	...
	*/
	async getTransaction(indexOrHash) {
		...
		if (typeof indexOrHash === "number") tx = this.#transactions[indexOrHash];
			...
			const hash$1 = indexOrHash.toLowerCase();
			for (const v of this.#transactions) if (typeof v === "string") {
				if (v !== hash$1) continue;
				tx = v;
				break;
			} else {
				if (v.hash !== hash$1) continue;
				tx = v;
				break;
			}
		}
...
var TransactionReceipt = class {
	...
	*/
	toJSON() {
		const { to, from, contractAddress, hash: hash$1, index, blockHash, blockNumber, logsBloom, logs, status, root } = this;
		return {
...
var TransactionResponse = class TransactionResponse {
	...
	*/
	toJSON() {
		const { blockNumber, blockHash, index, hash: hash$1, type, to, from, nonce, data, signature, accessList, blobVersionedHashes } = this;
		return {
	...
	async wait(_confirms, _timeout) {
		...
		const checkReplacement = async () => {
			...
			while (nextScan <= blockNumber) {
				...
				for (const hash$1 of block) if (hash$1 === this.hash) return;
				for (let i = 0; i < block.length; i++) {
					const tx = await block.getTransaction(i);
					if (tx.from === this.from && tx.nonce === this.nonce) {
						if (stopScanning) return null;
						const receipt$1 = await this.provider.getTransactionReceipt(tx.hash);
						if (receipt$1 == null) return;
						if (blockNumber - receipt$1.blockNumber + 1 < confirms) return;
						let reason = "replaced";
		...
		const checkReceipt = (receipt$1) => {
			if (receipt$1 == null || receipt$1.status !== 0) return receipt$1;
		...
		return await new Promise((resolve, reject) => {
			...
			const txListener = async (receipt$1) => {
				if (await receipt$1.confirmations() >= confirms) {
					cancel();
					try {
						resolve(checkReceipt(receipt$1));
					} catch (error) {
						reject(error);
					}
				}
...
async function getSubInfo(contract, event) {
	...
	if (Array.isArray(event)) {
		const topicHashify = function(name) {
			...
			const fragment$1 = contract.interface.getEvent(name);
			assertArgument(fragment$1, "unknown fragment", "name", name);
			return fragment$1.topicHash;
		};
		topics = event.map((e) => {
	...
	topics = topics.map((t) => {
		...
		if (Array.isArray(t)) {
			const items = Array.from(new Set(t.map((t$1) => t$1.toLowerCase())).values());
	...
	if (!sub) {
		...
		const listener = (log) => {
			...
			if (foundFragment) {
				...
				emit(contract, event, args, (listener$1) => {
					return new ContractEventPayload(contract, listener$1, event, _foundFragment, log);
				});
			} else emit(contract, event, [], (listener$1) => {
				return new ContractUnknownEventPayload(contract, listener$1, event, log);
			});
		};
		let starting = [];
		const start = () => {
...
var BaseContract = class BaseContract {
	...
	*/
	constructor(target, abi, runner, _deployTx) {
		...
		if (typeof target === "string") if (isHexString(target)) {
			...
			addrPromise = resolver.resolveName(target).then((addr$1) => {
				if (addr$1 == null) throw makeError("an ENS name used for a contract target must be correctly configured", "UNCONFIGURED_NAME", { value: target });
				getInternal(this).addr = addr$1;
				return addr$1;
			});
		}
		else addrPromise = target.getAddress().then((addr$1) => {
			if (addr$1 == null) throw new Error("TODO");
			getInternal(this).addr = addr$1;
			return addr$1;
		});
		setInternal(this, {
		...
		const filters = new Proxy({}, {
			get: (target$1, prop, receiver) => {
				if (typeof prop === "symbol" || passProperties.indexOf(prop) >= 0) return Reflect.get(target$1, prop, receiver);
				try {
			...
			has: (target$1, prop) => {
				if (passProperties.indexOf(prop) >= 0) return Reflect.has(target$1, prop);
				return Reflect.has(target$1, prop) || this.interface.hasEvent(String(prop));
			}
		});
		...
		return new Proxy(this, {
			get: (target$1, prop, receiver) => {
				if (typeof prop === "symbol" || prop in target$1 || passProperties.indexOf(prop) >= 0) return Reflect.get(target$1, prop, receiver);
				try {
					return target$1.getFunction(prop);
				} catch (error) {
					if (!isError(error, "INVALID_ARGUMENT") || error.argument !== "key") throw error;
				}
			},
			has: (target$1, prop) => {
				if (typeof prop === "symbol" || prop in target$1 || passProperties.indexOf(prop) >= 0) return Reflect.has(target$1, prop);
				return target$1.interface.hasFunction(prop);
			}
		});
	}
	/**
	...
	*/
	async off(event, listener) {
		...
		if (!sub) return this;
		if (listener) {
			const index = sub.listeners.map(({ listener: listener$1 }) => listener$1).indexOf(listener);
...
var ContractFactory = class ContractFactory {
	...
	*/
	static fromSolidity(output$1, runner) {
		assertArgument(output$1 != null, "bad compiler output", "output", output$1);
		if (typeof output$1 === "string") output$1 = JSON.parse(output$1);
		const abi = output$1.abi;
		let bytecode = "";
		if (output$1.bytecode) bytecode = output$1.bytecode;
		else if (output$1.evm && output$1.evm.bytecode) bytecode = output$1.evm.bytecode;
	...
	*/
	async getAddress(coinType) {
		...
		if (coinType >= 0 && coinType < 2147483648) {
			...
			const data$1 = await this.#fetch("addr(bytes32,uint)", [ethCoinType]);
			if (isHexString(data$1, 20)) return getAddress(data$1);
		}
		let coinPlugin = null;
					...
					case "erc1155": {
						...
						try {
							...
							try {
								...
								const bytes$1 = response.body;
								if (bytes$1) linkage.push({
									type: "!metadata",
									value: hexlify(bytes$1)
								});
								return {
...
function formatTransactionResponse(value) {
	...
	const result = object({
		...
		type: (value$1) => {
			if (value$1 === "0x" || value$1 == null) return 0;
	...
	*/
	static from(network) {
		...
		if (typeof network === "object") {
			...
			if (network.ensAddress || network.ensNetwork != null) custom.attachPlugin(new EnsPlugin(network.ensAddress, network.ensNetwork));
			return custom;
		}
...
function parseUnits$1(_value, decimals) {
...
function injectCommonNetworks() {
	...
	function registerEth(name, chainId, options) {
		const func = function() {
			...
			if (options.ensNetwork != null) network.attachPlugin(new EnsPlugin(null, options.ensNetwork));
		...
		if (options.altNames) options.altNames.forEach((name$1) => {
			Network.register(name$1, func);
		});
	}
	...
	async #poll() {
		try {
			...
			if (blockNumber !== this.#blockNumber) {
				for (let b$1 = this.#blockNumber + 1; b$1 <= blockNumber; b$1++) {
					if (this.#poller == null) return;
					await this.#provider.emit("block", b$1);
				}
				this.#blockNumber = blockNumber;
			}
		} catch (error) {}
...
async function getSubscription(_event, provider) {
	...
	if (isHexString(_event, 32)) {
		const hash$1 = _event.toLowerCase();
		return {
			type: "transaction",
			tag: getTag("tx", { hash: hash$1 }),
			hash: hash$1
		};
	}
	if (_event.orphan) {
	...
	if (_event.address || _event.topics) {
		...
		const filter = { topics: (event.topics || []).map((t) => {
			if (t == null) return null;
			if (Array.isArray(t)) return concisify(t.map((t$1) => t$1.toLowerCase()));
			return t.toLowerCase();
		}) };
		if (event.address) {
			const addresses = [];
...
var AbstractProvider = class {
	...
	*/
	_getBlockTag(blockTag) {
		...
		if (typeof blockTag === "number") {
			...
			return this.getBlockNumber().then((b$1) => toQuantity(b$1 + blockTag));
		}
	...
	*/
	_getFilter(filter) {
		...
		const resolve = (_address, fromBlock$1, toBlock$1) => {
			let address$1 = void 0;
			switch (_address.length) {
				case 0: break;
				case 1:
					address$1 = _address[0];
					break;
				default:
					_address.sort();
					address$1 = _address;
			...
			const filter$1 = {};
			if (address$1) filter$1.address = address$1;
			if (topics.length) filter$1.topics = topics;
			if (fromBlock$1) filter$1.fromBlock = fromBlock$1;
			if (toBlock$1) filter$1.toBlock = toBlock$1;
			if (blockHash) filter$1.blockHash = blockHash;
			return filter$1;
		};
		let address = [];
	...
	*/
		...
		try {
			...
			if (!this.disableCcipRead && isCallException(error) && error.data && attempt >= 0 && blockTag === "latest" && transaction.to != null && dataSlice(error.data, 0, 4) === "0x556f1830") {
				...
				try {
					...
					assert(false, error$1.message, "OFFCHAIN_FAULT", {
						reason: "BAD_DATA",
						transaction,
						info: { data }
					});
				...
				try {
					const result = await this.#call(tx$1, blockTag, attempt + 1);
					this.emit("debug", {
						action: "receiveCcipReadCallResult",
						transaction: Object.assign({}, tx$1),
						result
					});
					return result;
				} catch (error$1) {
	...
	async broadcastTransaction(signedTx) {
		const { blockNumber, hash: hash$1, network } = await resolveProperties({
		...
		if (tx.hash !== hash$1) throw new Error("@TODO: the returned hash did not match");
	...
	async getTransactionReceipt(hash$1) {
		const { network, params } = await resolveProperties({
			network: this.getNetwork(),
			params: this.#perform({
	...
	async getTransactionResult(hash$1) {
	...
	async lookupAddress(address) {
		...
		try {
			...
			const resolver = await new Contract(ensAddr, ["function resolver(bytes32) view returns (address)"], this).resolver(node);
			if (resolver == null || resolver === ZeroAddress) return null;
			const name = await new Contract(resolver, ["function name(bytes32) view returns (string)"], this).name(node);
			if (await this.resolveName(name) !== address) return null;
			return name;
		} catch (error) {
	...
	async waitForTransaction(hash$1, _confirms, timeout) {
		const confirms = _confirms != null ? _confirms : 1;
		if (confirms === 0) return this.getTransactionReceipt(hash$1);
		return new Promise(async (resolve, reject) => {
			let timer = null;
			const listener = (async (blockNumber) => {
				try {
					const receipt = await this.getTransactionReceipt(hash$1);
					if (receipt != null) {
...
function _parseString(result, start) {
	try {
		const bytes$1 = _parseBytes(result, start);
		if (bytes$1) return toUtf8String(bytes$1);
	} catch (error) {}
	return null;
}
...
async function populate(signer, tx) {
	...
	if (pop.from != null) {
		const from = pop.from;
		pop.from = Promise.all([signer.getAddress(), resolveAddress(from, signer)]).then(([address, from$1]) => {
			assertArgument(address.toLowerCase() === from$1.toLowerCase(), "transaction from mismatch", "tx.from", from$1);
			return address;
		});
...
var JsonRpcSigner = class extends AbstractSigner {
	...
	async sendTransaction(tx) {
		const blockNumber = await this.provider.getBlockNumber();
		const hash$1 = await this.sendUncheckedTransaction(tx);
		return await new Promise((resolve, reject) => {
			const timeouts = [1e3, 100];
			let invalids = 0;
			const checkTx = async () => {
				try {
					const tx$1 = await this.provider.getTransaction(hash$1);
					if (tx$1 != null) {
						resolve(tx$1.replaceableTransaction(blockNumber));
						return;
					}
				} catch (error) {
					if (isError(error, "CANCELLED") || isError(error, "BAD_DATA") || isError(error, "NETWORK_ERROR") || isError(error, "UNSUPPORTED_OPERATION")) {
						...
						error.info.sendTransactionHash = hash$1;
						reject(error);
						return;
					}
	...
	async signTypedData(domain, types, _value) {
		...
		const populated = await TypedDataEncoder.resolveNames(domain, types, value, async (value$1) => {
			const address = await resolveAddress(value$1);
			assertArgument(address != null, "TypedData does not support null address", "value", value$1);
			return address;
		});
			...
			while (payloads.length) {
				...
				(async () => {
					...
					try {
						...
						for (const { resolve, reject, payload: payload$1 } of batch) {
							if (this.destroyed) {
								reject(makeError("provider destroyed; cancelled request", "UNSUPPORTED_OPERATION", { operation: payload$1.method }));
								continue;
							}
							const resp = result.filter((r) => r.id === payload$1.id)[0];
							if (resp == null) {
							...
							if ("error" in resp) {
								reject(this.getRpcError(payload$1, resp));
								continue;
							}
							resolve(resp.result);
						}
	...
	*/
	async getSigner(address) {
		...
		if (typeof address === "number") {
			...
			if (address >= accounts$1.length) throw new Error("no such account");
			return new JsonRpcSigner(this, accounts$1[address]);
		}
...
var AnkrProvider = class AnkrProvider extends JsonRpcProvider {
	...
	*/
	static getRequest(network, apiKey) {
		...
		if (apiKey === defaultApiKey$1) request.retryFunc = async (request$1, response, attempt) => {
...
function getHost$4(name) {
	switch (name) {
		case "mainnet": return "eth-mainnet.alchemyapi.io";
...
var AlchemyProvider = class AlchemyProvider extends JsonRpcProvider {
	...
	static getRequest(network, apiKey) {
		...
		request.allowGzip = true;
		if (apiKey === defaultApiKey) request.retryFunc = async (request$1, response, attempt) => {
...
var ChainstackProvider = class ChainstackProvider extends JsonRpcProvider {
	...
	*/
	static getRequest(network, apiKey) {
		...
		request.allowGzip = true;
		if (apiKey === getApiKey(network.name)) request.retryFunc = async (request$1, response, attempt) => {
...
var EtherscanProvider = class extends AbstractProvider {
	...
	*/
	async fetch(module, params, post) {
		...
		request.processFunc = async (request$1, response$1) => {
			const result$1 = response$1.hasBody() ? JSON.parse(toUtf8String(response$1.body)) : {};
			const throttle = (typeof result$1.result === "string" ? result$1.result : "").toLowerCase().indexOf("rate limit") >= 0;
			if (module === "proxy") {
				if (result$1 && result$1.status == 0 && result$1.message == "NOTOK" && throttle) {
					this.emit("debug", {
						action: "receiveError",
						id: id$1,
						reason: "proxy-NOTOK",
					...
					response$1.throwThrottleError(result$1.result, THROTTLE);
				}
			} else if (throttle) {
				this.emit("debug", {
					action: "receiveError",
	...
	_checkError(req, error, transaction) {
		...
		if (message) {
			if (req.method === "broadcastTransaction") {
				const transaction$1 = Transaction.from(req.signedTransaction);
				if (message.match(/replacement/i) && message.match(/underpriced/i)) assert(false, "replacement fee too low", "REPLACEMENT_UNDERPRICED", { transaction: transaction$1 });
				if (message.match(/insufficient funds/)) assert(false, "insufficient funds for intrinsic transaction cost", "INSUFFICIENT_FUNDS", { transaction: transaction$1 });
				if (message.match(/same hash was already imported|transaction nonce is too low|nonce too low/)) assert(false, "nonce has already been used", "NONCE_EXPIRED", { transaction: transaction$1 });
			}
		}
		throw error;
	}
...
//#endregion
//#region ../node_modules/ethers/lib.esm/providers/ws-browser.js
function getGlobal() {
	if (typeof self !== "undefined") return self;
	if (typeof window !== "undefined") return window;
	if (typeof global !== "undefined") return global;
...
var InfuraProvider = class InfuraProvider extends JsonRpcProvider {
	...
	*/
	static getRequest(network, projectId, projectSecret) {
		...
		if (projectId === defaultProjectId) request.retryFunc = async (request$1, response, attempt) => {
...
var QuickNodeProvider = class QuickNodeProvider extends JsonRpcProvider {
	...
	*/
	static getRequest(network, token) {
		...
		request.allowGzip = true;
		if (token === defaultToken) request.retryFunc = async (request$1, response, attempt) => {
...
function stringify(value) {
	return JSON.stringify(value, (key, value$1) => {
		if (typeof value$1 === "bigint") return {
			type: "bigint",
...
async function waitForSync(config, blockNumber) {
	while (config.blockNumber < 0 || config.blockNumber < blockNumber) {
		if (!config._updateNumber) config._updateNumber = (async () => {
			try {
				const blockNumber$1 = await config.provider.getBlockNumber();
				if (blockNumber$1 > config.blockNumber) config.blockNumber = blockNumber$1;
			} catch (error) {
...
function getMedian(quorum, results) {
	...
	values.sort((a, b$1) => a < b$1 ? -1 : b$1 > a ? 1 : 0);
...
var FallbackProvider = class extends AbstractProvider {
	...
	*/
	...
	#getNextConfig(running) {
		...
		shuffle(allConfigs);
		allConfigs.sort((a, b$1) => a.priority - b$1.priority);
	...
	async _perform(req) {
		if (req.method === "broadcastTransaction") {
			...
			const broadcasts = this.#configs.map(async ({ provider, weight }, index) => {
				try {
					const result$2 = await provider._perform(req);
					results[index] = Object.assign(normalizeResult(req.method, { result: result$2 }), { weight });
				} catch (error) {
			...
			while (true) {
				...
				const result$2 = checkQuorum(this.quorum, results.filter((r) => r != null));
				if (isError(result$2, "INSUFFICIENT_FUNDS")) throw result$2;
				const waiting = broadcasts.filter((b$1, i) => results[i] == null);
				if (waiting.length === 0) break;
				await Promise.race(waiting);
			}
			const result$1 = getAnyResult(this.quorum, results);
			assert(result$1 !== void 0, "problem multi-broadcasting", "SERVER_ERROR", {
				request: "%sub-requests",
				info: {
					request: req,
			...
			const finalize = () => {
				...
				if (found.length) if (options && options.filter) {
					...
					if (filtered == null) resolve(null);
						...
						let match = null;
						if (filtered.uuid) match = found.filter((f$1) => filtered.uuid === f$1.info.uuid)[0];
						if (match) {
...
var PocketProvider = class PocketProvider extends JsonRpcProvider {
	...
	*/
	static getRequest(network, applicationId, applicationSecret) {
		...
		if (applicationId === defaultApplicationId) request.retryFunc = async (request$1, response, attempt) => {
...
//#endregion
//#region ../node_modules/ethers/lib.esm/providers/provider-ipcsocket-browser.js
...
var subsChrs = " !#$%&'()*+,-./<=>?@[]^_`{|}~";
var Word = /^[a-z]*$/i;
function unfold(words$1, sep) {
	let initial = 97;
	return words$1.reduce((accum, word) => {
		if (word === sep) initial++;
...
var WordlistOwl = class extends Wordlist {
	...
	*/
	constructor(locale, data, checksum$1) {
		super(locale);
		this.#data = data;
	...
	*/
	...
	#loadWords() {
		if (this.#words == null) {
			const words$1 = this._decodeWords();
			/* c8 ignore start */
	...
	getWord(index) {
		const words$1 = this.#loadWords();
		assertArgument(index >= 0 && index < words$1.length, `invalid word index: ${index}`, "index", index);
...
function mnemonicToEntropy(mnemonic, wordlist$1) {
	assertNormalize("NFKD");
	if (wordlist$1 == null) wordlist$1 = LangEn.wordlist();
	const words$1 = wordlist$1.split(mnemonic);
	assertArgument(words$1.length % 3 === 0 && words$1.length >= 12 && words$1.length <= 24, "invalid mnemonic length", "mnemonic", "[ REDACTED ]");
	const entropy = new Uint8Array(Math.ceil(11 * words$1.length / 8));
	let offset = 0;
	for (let i = 0; i < words$1.length; i++) {
		let index = wordlist$1.getWordIndex(words$1[i].normalize("NFKD"));
	...
	const entropyBits = 32 * words$1.length / 3;
	const checksumBits = words$1.length / 3;
	const checksumMask = getUpperMask(checksumBits);
	const checksum$1 = getBytes(sha256(entropy.slice(0, entropyBits / 8)))[0] & checksumMask;
	assertArgument(checksum$1 === (entropy[entropy.length - 1] & checksumMask), "invalid mnemonic checksum", "mnemonic", "[ REDACTED ]");
	return hexlify(entropy.slice(0, entropyBits / 8));
}
function entropyToMnemonic(entropy, wordlist$1) {
	...
	const checksum$1 = parseInt(sha256(entropy).substring(2, 4), 16) & getUpperMask(checksumBits);
	indices[indices.length - 1] <<= checksumBits;
	indices[indices.length - 1] |= checksum$1 >> 8 - checksumBits;
	return wordlist$1.join(indices.map((index) => wordlist$1.getWord(index)));
}
var _guard$1 = {};
/**
...
var Mnemonic = class Mnemonic {
	...
	*  @private
	*/
	constructor(guard, entropy, phrase, password, wordlist$1) {
	...
	*/
	static fromPhrase(phrase, password, wordlist$1) {
		const entropy = mnemonicToEntropy(phrase, wordlist$1);
		phrase = entropyToMnemonic(getBytes(entropy), wordlist$1);
		return new Mnemonic(_guard$1, entropy, phrase, password, wordlist$1);
	}
	/**
	...
	*  wordlist is the [English wordlists](LangEn).
	*/
	static fromEntropy(_entropy, password, wordlist$1) {
		const entropy = getBytes(_entropy, "entropy");
		const phrase = entropyToMnemonic(entropy, wordlist$1);
		return new Mnemonic(_guard$1, hexlify(entropy), phrase, password, wordlist$1);
	}
	/**
	*  Returns the phrase for %%mnemonic%%.
	*/
	static entropyToPhrase(_entropy, wordlist$1) {
		const entropy = getBytes(_entropy, "entropy");
		return entropyToMnemonic(entropy, wordlist$1);
	}
	/**
	*  Returns the entropy for %%phrase%%.
	*/
	static phraseToEntropy(phrase, wordlist$1) {
		return mnemonicToEntropy(phrase, wordlist$1);
	}
	/**
	...
	*/
	static isValidMnemonic(phrase, wordlist$1) {
		try {
			mnemonicToEntropy(phrase, wordlist$1);
			return true;
		} catch (error) {}
		return false;
	}
...
//#endregion
//#region ../node_modules/aes-js/lib.esm/aes.js
/*! MIT License. Copyright 2015-2022 Richard Moore <me@ricmoo.com>. See LICENSE.txt. */
var __classPrivateFieldGet$2 = void 0 && (void 0).__classPrivateFieldGet || function(receiver, state, kind, f$1) {
...
var __classPrivateFieldSet$2 = void 0 && (void 0).__classPrivateFieldSet || function(receiver, state, value, kind, f$1) {
...
function convertToInt32(bytes$1) {
	const result = [];
	for (let i = 0; i < bytes$1.length; i += 4) result.push(bytes$1[i] << 24 | bytes$1[i + 1] << 16 | bytes$1[i + 2] << 8 | bytes$1[i + 3]);
	return result;
}
var AES = class AES {
	get key() {
	...
	constructor(key) {
		...
		while (t < roundKeyCount) {
			...
			rconpointer += 1;
			if (KC != 8) for (let i$1 = 1; i$1 < KC; i$1++) tk[i$1] ^= tk[i$1 - 1];
			else {
				for (let i$1 = 1; i$1 < KC / 2; i$1++) tk[i$1] ^= tk[i$1 - 1];
				tt = tk[KC / 2 - 1];
				tk[KC / 2] ^= S[tt & 255] ^ S[tt >> 8 & 255] << 8 ^ S[tt >> 16 & 255] << 16 ^ S[tt >> 24 & 255] << 24;
				for (let i$1 = KC / 2 + 1; i$1 < KC; i$1++) tk[i$1] ^= tk[i$1 - 1];
			}
			let i = 0, r, c;
	...
	encrypt(plaintext) {
		...
		for (let i = 0; i < 4; i++) t[i] ^= __classPrivateFieldGet$2(this, _AES_Ke, "f")[0][i];
		for (let r = 1; r < rounds; r++) {
			for (let i = 0; i < 4; i++) a[i] = T1[t[i] >> 24 & 255] ^ T2[t[(i + 1) % 4] >> 16 & 255] ^ T3[t[(i + 2) % 4] >> 8 & 255] ^ T4[t[(i + 3) % 4] & 255] ^ __classPrivateFieldGet$2(this, _AES_Ke, "f")[r][i];
			t = a.slice();
		}
	...
	decrypt(ciphertext) {
		...
		for (let i = 0; i < 4; i++) t[i] ^= __classPrivateFieldGet$2(this, _AES_Kd, "f")[0][i];
		for (let r = 1; r < rounds; r++) {
			for (let i = 0; i < 4; i++) a[i] = T5[t[i] >> 24 & 255] ^ T6[t[(i + 3) % 4] >> 16 & 255] ^ T7[t[(i + 2) % 4] >> 8 & 255] ^ T8[t[(i + 1) % 4] & 255] ^ __classPrivateFieldGet$2(this, _AES_Kd, "f")[r][i];
			t = a.slice();
		}
		const result = new Uint8Array(16);
		let tt = 0;
...
//#endregion
//#region ../node_modules/aes-js/lib.esm/mode.js
...
//#endregion
//#region ../node_modules/aes-js/lib.esm/mode-cbc.js
...
var CBC = class CBC extends ModeOfOperation {
	constructor(key, iv) {
		...
		} else __classPrivateFieldSet$1(this, _CBC_iv, new Uint8Array(16), "f");
	...
	encrypt(plaintext) {
		...
		for (let i = 0; i < plaintext.length; i += 16) {
			for (let j = 0; j < 16; j++) __classPrivateFieldGet$1(this, _CBC_lastBlock, "f")[j] ^= plaintext[i + j];
...
//#endregion
//#region ../node_modules/aes-js/lib.esm/mode-ctr.js
...
//#endregion
//#region ../node_modules/aes-js/lib.esm/padding.js
function pkcs7Strip(data) {
	...
	for (let i = 0; i < padder; i++) if (data[length + i] !== padder) throw new TypeError("PKCS#7 invalid padding byte");
...
function spelunk(object$1, _path) {
...
function getDecryptKdfParams(data) {
	...
	if (kdf && typeof kdf === "string") {
		if (kdf.toLowerCase() === "scrypt") {
			...
			const N$2 = spelunk(data, "crypto.kdfparams.n:int!");
			const r = spelunk(data, "crypto.kdfparams.r:int!");
			const p = spelunk(data, "crypto.kdfparams.p:int!");
			assertArgument(N$2 > 0 && (N$2 & N$2 - 1) === 0, "invalid kdf.N", "kdf.N", N$2);
...
function decryptKeystoreJsonSync(json, _password) {
	...
	if (params.name === "pbkdf2") {
		const { salt: salt$1, count, dkLen: dkLen$1, algorithm } = params;
		const key$1 = pbkdf2(password, salt$1, count, dkLen$1, algorithm);
		return getAccount(data, key$1);
	}
	assert(params.name === "scrypt", "cannot be reached", "UNKNOWN_ERROR", { params });
	const { salt, N: N$2, r, p, dkLen } = params;
	const key = scryptSync(password, salt, N$2, r, p, dkLen);
...
async function decryptKeystoreJson(json, _password, progress) {
	...
	assert(params.name === "scrypt", "cannot be reached", "UNKNOWN_ERROR", { params });
	const { salt, N: N$2, r, p, dkLen } = params;
	const key = await scrypt(password, salt, N$2, r, p, dkLen, progress);
	return getAccount(data, key);
}
function getEncryptKdfParams(options) {
	const salt = options.salt != null ? getBytes(options.salt, "options.salt") : randomBytes(32);
	let N$2 = 1 << 17, r = 8, p = 1;
	if (options.scrypt) {
		if (options.scrypt.N) N$2 = options.scrypt.N;
		if (options.scrypt.r) r = options.scrypt.r;
		if (options.scrypt.p) p = options.scrypt.p;
	}
	assertArgument(typeof N$2 === "number" && N$2 > 0 && Number.isSafeInteger(N$2) && (BigInt(N$2) & BigInt(N$2 - 1)) === BigInt(0), "invalid scrypt N parameter", "options.N", N$2);
...
function _encryptKeystore(key, kdf, account, options) {
	...
	if (account.mnemonic) {
		...
		const gethFilename = "UTC--" + (now.getUTCFullYear() + "-" + zpad$1(now.getUTCMonth() + 1, 2) + "-" + zpad$1(now.getUTCDate(), 2) + "T" + zpad$1(now.getUTCHours(), 2) + "-" + zpad$1(now.getUTCMinutes(), 2) + "-" + zpad$1(now.getUTCSeconds(), 2) + ".0Z") + "--" + data.address;
		data["x-ethers"] = {
			client,
			gethFilename,
			path,
...
function encodeBase58Check(_value) {
	...
	const bytes$1 = concat([value, check]);
	return encodeBase58(bytes$1);
}
var _guard = {};
...
var HDNodeWallet = class HDNodeWallet extends BaseWallet {
	...
	*/
	deriveChild(_index) {
		...
		const ki = new SigningKey(toBeHex((toBigInt(IL) + BigInt(this.privateKey)) % N$1, 32));
	...
	*/
	static fromExtendedKey(extendedKey) {
		const bytes$1 = toBeArray(decodeBase58(extendedKey));
		assertArgument(bytes$1.length === 82 || encodeBase58Check(bytes$1.slice(0, 78)) === extendedKey, "invalid extended key", "extendedKey", "[ REDACTED ]");
		const depth = bytes$1[4];
		const parentFingerprint = hexlify(bytes$1.slice(5, 9));
		const index = parseInt(hexlify(bytes$1.slice(9, 13)).substring(2), 16);
		const chainCode = hexlify(bytes$1.slice(13, 45));
		const key = bytes$1.slice(45, 78);
		switch (hexlify(bytes$1.slice(0, 4))) {
			case "0x0488b21e":
			case "0x043587cf": {
	...
	*/
	static createRandom(password, path, wordlist$1) {
		if (password == null) password = "";
		if (path == null) path = defaultPath;
		if (wordlist$1 == null) wordlist$1 = LangEn.wordlist();
		const mnemonic = Mnemonic.fromEntropy(randomBytes(16), password, wordlist$1);
	...
	*/
	static fromPhrase(phrase, password, path, wordlist$1) {
		if (password == null) password = "";
		if (path == null) path = defaultPath;
		if (wordlist$1 == null) wordlist$1 = LangEn.wordlist();
		const mnemonic = Mnemonic.fromPhrase(phrase, password, wordlist$1);
		return HDNodeWallet.#fromSeed(mnemonic.computeSeed(), mnemonic).derivePath(path);
	}
	/**
	...
	*/
	...
	static #fromAccount(account) {
		...
		if ("mnemonic" in account && account.mnemonic && account.mnemonic.locale === "en") {
			...
			const wallet$1 = HDNodeWallet.fromMnemonic(mnemonic, account.mnemonic.path);
			if (wallet$1.address === account.address && wallet$1.privateKey === account.privateKey) return wallet$1;
...
function decodeOwlA(data, accents) {
	let words$1 = decodeOwl(data).join(",");
	accents.split(/,/g).forEach((accent) => {
		...
		words$1 = words$1.replace(regex, (all, letter) => {
...
var WordlistOwlA = class extends WordlistOwl {
	...
	*/
	constructor(locale, data, accent, checksum$1) {
		super(locale, data, checksum$1);
		this.#accent = accent;
	}
	/**
...
//#endregion
//#region ../node_modules/ethers/lib.esm/wordlists/wordlists-browser.js
const wordlists = { en: LangEn.wordlist() };

//#endregion
//#region ../node_modules/ethers/lib.esm/ethers.js
var ethers_exports = /* @__PURE__ */ __export({
	AbiCoder: () => AbiCoder,
